...
class _Model3DPageState extends State<Model3DPage>
  ...
  String? _matchPart(String name) {
    ...
    if (lower.contains('light') || lower.contains('lamp') || lower.contains('led') || lower.contains('head') || lower.contains('集合2')) return 'light';
...
loader.load(url,gltf=>{model=gltf.scene;model.traverse(n=>{if(n.isMesh){n.castShadow=true;n.receiveShadow=true}});scene.add(model);document.getElementById('loading').style.display='none';modelLoaded.postMessage('ok');URL.revokeObjectURL(url)});
const raycaster=new THREE.Raycaster();
let _hl=[];renderer.domElement.addEventListener('click',e=>{if(!model)return;const rect=renderer.domElement.getBoundingClientRect();const mouse=new THREE.Vector2(((e.clientX-rect.left)/rect.width)*2-1,-((e.clientY-rect.top)/rect.height)*2+1);raycaster.setFromCamera(mouse,camera);const hits=raycaster.intersectObjects(model.children,true);if(hits.length>0){let obj=hits[0].object;let meshName=null;let inLight=false;while(obj&&obj!==model){if(obj.name&&obj.name.length>0){if(obj.name==='集合2')inLight=true;if(!meshName)meshName=obj.name}obj=obj.parent}if(inLight)partSelected.postMessage('集合2');else if(meshName)partSelected.postMessage(meshName)}});window.toggleAutoRotate=v=>{controls.autoRotate=v};window.resetView=()=>{camera.position.set(1.2,0.6,1.2);controls.target.set(0,0,0);controls.update()};window.highlightPart=name=>{if(!model)return;clearHighlight();const lower=name.toLowerCase();const out=[];if(lower==='light'){model.traverse(n=>{if(n.name==='集合2'){n.traverse(c=>{if(c.isMesh)out.push(c)})}})}else{model.traverse(n=>{if(!n.isMesh||!n.name)return;const nl=n.name.toLowerCase();if(nl.includes(lower))out.push(n);if(lower==='lidar'&&nl.includes('laser_link'))out.push(n);if(lower==='wheel'&&(nl.includes('wheel')||nl.includes('左')||nl.includes('右')))out.push(n)})}out.forEach(n=>{n.material.emissive=new THREE.Color(0xe88b45);n.material.emissiveIntensity=0.6});_hl=out};window.clearHighlight=()=>{_hl.forEach(n=>{n.material.emissive.set(0x000000);n.material.emissiveIntensity=0});_hl=[]};
