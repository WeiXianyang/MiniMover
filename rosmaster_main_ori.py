...
g_capture_image = False
g_capture_video = False
g_video_writer = None
g_video_file_name = None
...
def parse_data(sk_client, data):
    ...
    global g_capture_image
    global g_capture_video
    ...
    if cmd == "0F":  # 进入界面
        ...
        if g_debug:
            print("num_x:"+str(num_x))
            print("num_y:"+str(num_y))
        if num_x > 127:
            num_x = num_x - 256
        if num_y > 127:
            num_y = num_y - 256
        ...
        if speed_x == 0 and speed_y == 0:
            print('speed_x=0 and speed_y=0')
            if g_car_type == g_bot.CARTYPE_R2:
                g_bot.set_car_run(0, g_car_stabilize_state, 1)
            else:
                print("g_car_stabilize_state="+str(g_car_stabilize_state))
                g_bot.set_car_run(0, g_car_stabilize_state)
        else:
            if g_car_type == g_bot.CARTYPE_R2:
    ...
    # 保存单张图片
        ...
        g_capture_image = True
        print("will save one image")
    
    # 开始录制视频
    elif cmd == '61': 
        g_capture_video = True
        print("will start to save video")

    # 结束录制视频
    elif cmd == '62':
        g_capture_video = False
        print("will stop to save video")

    # 启动巡线功能
    elif cmd == '63':
        print("start to follow the line")
        g_bot.set_follow_line(1)
    
    # 终止巡线功能
    elif cmd == '64':
        print("stop to follow the line")
        g_bot.set_follow_line(0)
...
def capture_img(frame):
    global g_capture_image
    if g_capture_image == True:
        ...
        fileName = img_dir + dt + '.jpg'
        cv.imwrite(fileName, frame)
        print(f'Capture image file: {fileName}')
        g_capture_image = False

def capture_video(frame):
    global g_capture_video
    global g_video_writer
    global g_video_file_name

    if g_capture_video == True:
        if g_video_writer is None:
            dt = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
            img_dir  = './capture/'
            if not os.path.exists(img_dir):
                os.makedirs(img_dir)
                print(f"Directory {img_dir} created.")
            g_video_file_name = img_dir + dt + '.avi'
            fourcc = cv.VideoWriter_fourcc(*'XVID')
            g_video_writer = cv.VideoWriter(g_video_file_name, fourcc, 20.0, (640, 480))
 
        g_video_writer.write(frame)
    else:
        if g_video_writer is not None:
            print(f'Save avi file: {g_video_file_name}')
...
def mode_handle():
    ...
    global g_camera, g_camera_usb, g_camera_wide_angle
    if g_debug:
        print("----------------------------mode_handle--------------------------")
    while True:
        m_fps = 0
        ...
        while True:
            ...
            if True:
                ...
                elif g_camera_type == g_camera.TYPE_WIDE_ANGLE_CAMERA:
                    success, frame = g_camera_wide_angle.get_frame()
                else:
                ...
                if not success:
                    ...
                    if g_car_type == g_bot.CARTYPE_R2:
                        if g_camera_type == g_camera.TYPE_WIDE_ANGLE_CAMERA:
                            g_camera_type = g_camera.TYPE_DEPTH_CAMERA
                        else:
                            g_camera_type = g_camera.TYPE_WIDE_ANGLE_CAMERA
                    if g_camera_type == g_camera.TYPE_USB_CAMERA:
                        g_camera_usb.reconnect()
                    elif g_camera_type == g_camera.TYPE_WIDE_ANGLE_CAMERA:
                        g_camera_wide_angle.reconnect()
                    else:
                        g_camera.reconnect()
                    time.sleep(.5)
                    continue
