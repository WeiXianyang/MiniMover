...
**已接受，并按 2026-07-17 比赛演示简化规则修订。**

当前实现以 Nav2 action 的 `SUCCEEDED` 作为运动完成权威结果；`map` 位姿和距离只用于诊断。比赛演示只要求一名安全员、路线清场和硬件急停，不采用旧版双人复核/连续三次放行流程。
...
6. Nav2 返回 `SUCCEEDED` 后小车停止，并播报“已到达内科，请注意脚下。”，同时在控制台留下可查看的状态与位姿诊断。
...
stateDiagram-v2
  ...
  NAVIGATING --> ARRIVED: Nav2 action 返回 SUCCEEDED
...
- “到达”以 Nav2 action 的 `SUCCEEDED` 为准；仅收到目标或处于 `ACTIVE` 不算到达。地图位姿与距离只作诊断，不再以额外 `0.15 m` 门槛否决 Nav2 成功。
- 任何异常均禁止伪造“已到达内科”播报。
...
4. 将实车验收通过的坐标写入 `internal_medicine.navigation`；仅该条目设置 `enabled=true`，并由一名安全员在清场后确认可达性。
5. 在无观众且路线清场时完成一次实体端到端验收；安全员全程持有硬件急停。
...
- 从正式起点完成一次完整实体验收：“识别/访客回退 → 内科确认 → 行驶与避障 → Nav2 成功 → 停车与到达播报”。
...
| 地图漂移或初始位姿错误 | 小车偏离路线 | 每次开始前检查 map 位姿与固定起点；异常时立即使用硬件急停。 |
