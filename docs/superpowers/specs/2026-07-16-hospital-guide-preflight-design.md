...
**已实现。** 本文记录初代演示所用的只读现场预检脚本；脚本只确认接入状态，不改变小车、导航或导诊运行行为。

## 背景与目标

现有 runbook 已列出 Nav2 action、`/api/nav/pose` 和控制台的人工检查命令。实际现场容易遗漏 ROS 域、API 地址或定位状态，且不能用“页面能打开”替代导航放行证据。

新增 `scripts/check_hospital_guide_preflight.sh`，将这些检查收敛为一次**只读**预检。它的目标是为安全操作员提供简洁、可保存的“通过 / 未通过 / 无法检查”报告；它不取代一名安全员、硬件急停和路线清场的人工确认。
...
`internal_medicine.navigation.enabled` 由已验收的比赛配置控制，预检脚本无权修改它。
...
[PENDING] release_gate: safety officer, hardware emergency stop, and route clearance must be confirmed
```
...
- runbook 明确注明：预检通过后仍需一名安全员、硬件急停和路线清场确认。
