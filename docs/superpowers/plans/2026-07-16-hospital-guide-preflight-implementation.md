...
该工具只执行固定 API 的 `GET` 与 ROS 图发现；它不会启动、停止、取消导航或发送底盘控制。预检通过只说明自动检查项可见，**不等于实车放行**。仍须确认一名安全员、硬件急停和路线清场，才可开始实体演示。
```
